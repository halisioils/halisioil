...
const DeleteProducts = () => {
  ...
  const id = searchParams.get("product_delete");
  const url_images = searchParams.get("product_images");
  ...
  const deleteProject = api.product.delete.useMutation({
    onSuccess: async () => {
  ...
  return (
    <section className="relative flex w-[90vw] flex-col items-center justify-center gap-[1.5rem] rounded-[0.5rem] bg-white px-[2.5rem] py-[2rem] md:w-[400px]">
      ...
      <div className="flex w-[100%] items-center justify-center gap-[0.5rem] text-[0.875rem]">
        ...
        >
          {deleteProject.isPending ? <LoadingComponent /> : "Yes, Confirm"}
        </button>
      </div>
    </section>
  );
